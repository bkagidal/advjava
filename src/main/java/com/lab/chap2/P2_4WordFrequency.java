...
//Program P2.4
        ...
        public class P2_4WordFrequency {
           ...
           public static int binarySearch(String key, WordInfo1[] list, int lo, int hi) {
           ...
           public static void addToList(String item, WordInfo1[] list, int p, int n) {
           //sets list[p].word to item; sets list[p].freq to 1
           //shifts list[n] down to list[p] to the right
              for (int h = n; h >= p; h--) list[h + 1] = list[h];
              list[p] = new WordInfo1(item, 1);
           } //end addToList

           public static void printResults(PrintWriter out, WordInfo1[] list, int n) {
              out.printf("\nWords             Frequency\n\n");
              for (int h = 0; h < n; h++)
